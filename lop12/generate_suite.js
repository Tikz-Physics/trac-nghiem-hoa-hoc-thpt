  ...
  return `
    ...
    *, button, input, select, textarea { box-sizing: border-box; margin: 0; padding: 0; font-family: var(--font-family); -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; -webkit-tap-highlight-color: transparent; }
    ...
    .header-bar {
      ...
      scrollbar-color: rgba(6, 182, 212, 0.45) rgba(255, 255, 255, 0.04);
      padding-bottom: 3px;
      -webkit-overflow-scrolling: touch;
    }
    .header-bar.is-dragging {
    ...
    .header-bar::-webkit-scrollbar-thumb:hover {
    ...
    .graph-box {
      ...
      border-radius: 14px;
      padding: 12px;
      box-shadow: 0 8px 25px rgba(0,0,0,0.5), inset 0 0 15px rgba(6, 182, 212, 0.08);
      width: 100%;
      height: 100%;
      flex: 1;
      display: flex;
    ...
    .tf-btn { 
      padding: 5px 12px; border-radius: 6px; font-size: calc(0.85rem * var(--font-scale)); font-weight: 700; cursor: pointer; 
    ...
    .match-card {
      ...
      padding: 8px 12px; font-size: calc(0.90rem * var(--font-scale)); cursor: pointer; transition: 0.15s; line-height: 1.4;
    }
    ...
    .word-chip { background: #4f46e5; padding: 6px 14px; border-radius: 999px; font-weight: 700; font-size: calc(0.86rem * var(--font-scale)); cursor: pointer; border: none; color: #fff; transition: 0.15s; }
    ...
    .btn-exp-zoom:active { transform: scale(0.96); }
    .btn-exp-expand {
    ...
    /* Explanation body text scale */
    .exp-body-content {
      font-size: calc(0.9rem * var(--font-scale) * var(--exp-font-scale, 1));
      line-height: 1.68;
      color: #f1f5f9;
      transition: font-size 0.15s ease;
    }
    .exp-body-content b { color: #38bdf8; }
    .exp-body-content .math-sym { color: #38bdf8; }

    /* Zoomed modal state for explanation panel */
    .explanation-panel.is-zoomed {
      position: fixed;
      top: 50%;
      left: 50%;
      ...
      max-height: 85vh;
      z-index: 10000;
      box-shadow: 0 25px 70px rgba(0, 0, 0, 0.85), 0 0 0 100vmax rgba(4, 7, 15, 0.82);
      ...
      animation: zoomInExp 0.22s cubic-bezier(0.16, 1, 0.3, 1);
    }
    .explanation-panel.is-zoomed .exp-title-row {
      font-size: 1.1rem;
      padding-bottom: 8px;
      margin-bottom: 12px;
      border-bottom: 1.5px solid rgba(56, 189, 248, 0.3);
    }
    .explanation-panel.is-zoomed .answer-badge-card {
      font-size: 1.05rem;
      padding: 7px 14px;
      margin-bottom: 12px;
    }
    .explanation-panel.is-zoomed .exp-body-content {
      font-size: calc(1.08rem * var(--font-scale) * var(--exp-font-scale, 1));
      line-height: 1.85;
    }
    @keyframes zoomInExp {
  ...
  <div class="app-modal" id="teacherModal" style="display:none;" onclick="if(event.target===this) closeTeacherModal()">
    <div class="modal-card" onclick="event.stopPropagation()">
      ...
      <div class="modal-body">
        ...
        <input type="text" id="teacherNameInput" class="custom-input" placeholder="Ví dụ: Thầy Nam, Cô Lan..." />
...
function getBaseJS() {
  return `
// SafeStorage Engine for Mobile & file:// compatibility (Fallback to memory if localStorage is restricted)
    const SafeStorage = {
      _mem: {},
      getItem(key) {
        try {
          const val = window.SafeStorage.getItem(key);
          return val !== null ? val : (this._mem[key] !== undefined ? this._mem[key] : null);
        } catch (e) {
      ...
      setItem(key, val) {
        ...
        try {
          window.SafeStorage.setItem(key, str);
        } catch (e) {}
        this._mem[key] = str;
      },
      removeItem(key) {
...
// Unlock Web Audio on first mobile touch or click
    ...
    function initFontScale() {
      try {
        const saved = SafeStorage.getItem('HOA12_FONT_SCALE_IDX');
    ...
    function changeFontScale(delta) {
      ...
      try {
        SafeStorage.setItem('HOA12_FONT_SCALE_IDX', currentScaleIdx);
      } catch(e) {}
      applyFontScale();
    }
    ...
    // Horizontal Wheel & Drag Scroll for Header Bar
    function initHeaderScroll() {
      ...
      headerBars.forEach(bar => {
        ...
        bar.addEventListener('mousedown', (e) => {
          if (e.target.closest('button, select, a, input, .teacher-badge')) return;
          isDown = true;
    ...
    function initTeacherName() {
      try {
        ...
        if (paramTeacher && paramTeacher.trim()) {
          ...
          SafeStorage.setItem('HOA12_CUSTOM_TEACHER', teacherName);
        } else {
          const saved = SafeStorage.getItem('HOA12_CUSTOM_TEACHER');
...
function getQuestionRenderersJS() {
  return `
    ...
    /* Save user answers in Exam Mode */
    function saveExamAnswer(qIdx, val) {
      ...
      if (q.type === 'mcq') {
        ...
        let correctCount = 0;
        q.stmts.forEach((st, sIdx) => {
          const expected = (q.ans && Array.isArray(q.ans)) ? q.ans[sIdx] : (st.a !== undefined ? st.a : null);
          if (val[sIdx] === expected) correctCount++;
        });
        ...
        q.ans.forEach((ansTarget, aIdx) => {
          ...
          if (val[aIdx] === expectedWord) correctSlots++;
        });
    ...
    function renderExplanationHTML(q, badgeClass, statusIcon) {
      ...
      if (q.type === 'mcq' && typeof q.ans === 'number') {
        const correctLetter = ['A', 'B', 'C', 'D'][q.ans] || 'A';
        expText = expText.replace(/Chọn đáp án [A-D]/g, 'Chọn đáp án ' + correctLetter);
      }
      return '<div class="explanation-panel" id="expPanel">' +
        ...
        '<div class="exp-title-row">' +
          ...
          '<div class="exp-zoom-controls">' +
            ...
            '<button type="button" class="btn-exp-zoom" onclick="changeExpFontSize(1)" title="Phóng to cỡ chữ (A+)">A+</button>' +
            '<button type="button" class="btn-exp-zoom btn-exp-expand" id="btnExpExpand" onclick="toggleExpZoom(event)" title="Phóng to / Thu nhỏ toàn màn hình">' +
              '<span id="expExpandIcon">⛶</span> <span id="expExpandLabel">Phóng to</span>' +
            '</button>' +
          '</div>' +
        '</div>' +
        '<div class="exp-body-content" id="expBodyContent">' + expText + '</div>' +
      '</div>';
    }
    ...
    function checkAnswer(idx) {
      ...
      if (q.type === 'mcq') {
        ...
        q.stmts.forEach((st, sIdx) => {
          ...
          const expected = (q.ans && Array.isArray(q.ans)) ? q.ans[sIdx] : (st.a !== undefined ? st.a : null);
          const isItemCorrect = (userVal === expected);
      ...
      const expHtml = renderExplanationHTML(q, badgeClass, statusIcon);
    ...
    // Explanation Zoom Engine
    const EXP_FONT_SCALES = [0.85, 1.00, 1.15, 1.30, 1.50];
    let expScaleIdx = 1;

    function initExpZoom() {
      try {
        const saved = SafeStorage.getItem('VT_EXP_FONT_SCALE');
        if (saved !== null && !isNaN(parseInt(saved, 10))) {
          expScaleIdx = Math.max(0, Math.min(EXP_FONT_SCALES.length - 1, parseInt(saved, 10)));
        }
      } catch(e) {}
      applyExpFontSize();
    }

    function changeExpFontSize(delta) {
      AudioEngine.playClick();
      expScaleIdx = Math.max(0, Math.min(EXP_FONT_SCALES.length - 1, expScaleIdx + delta));
      try {
        SafeStorage.setItem('VT_EXP_FONT_SCALE', expScaleIdx);
      } catch(e) {}
      applyExpFontSize();
    }

    function applyExpFontSize() {
      const scale = EXP_FONT_SCALES[expScaleIdx];
      document.documentElement.style.setProperty('--exp-font-scale', scale);
      const display = document.getElementById('expZoomLevelDisplay');
      if (display) display.textContent = Math.round(scale * 100) + '%';
    }

    function toggleExpZoom(e) {
      if (e) e.stopPropagation();
      AudioEngine.playClick();
      const panel = document.getElementById('expPanel');
      if (!panel) return;
      const isZoomed = panel.classList.toggle('is-zoomed');
      const icon = document.getElementById('expExpandIcon');
      const label = document.getElementById('expExpandLabel');
      if (isZoomed) {
    ...
    document.addEventListener('click', (e) => {
      const panel = document.getElementById('expPanel');
      if (panel && panel.classList.contains('is-zoomed')) {
    ...
    function initKeyboardNavigation() {
      window.addEventListener('keydown', (e) => {
        // Ignore navigation when typing in form inputs or dropdowns
        ...
        if (teacherModal && teacherModal.style.display === 'flex') return;

        // Ignore if result card or dashboard/exam review is open
        const resCard = document.getElementById('resultCard');
        if (resCard && resCard.style.display === 'block') return;
        ...
        // ArrowRight: Tiến tới câu sau (nextQuestion)
          ...
          if (currentQ < questions.length - 1) {
            ...
            const expPh = document.getElementById('explanationPlaceholder');
...
<head>
  ...
  <title>Hệ Thống Trắc Nghiệm Hoá Học 12</title>
...
<body>
      ...
      <div class="header-bar">
        ...
        <div class="header-group">
          <div class="teacher-badge" id="teacherBadge" onclick="openTeacherModal()" title="Nhấp để thay đổi tên giáo viên">
            <span>👨‍🏫</span>
            <span id="teacherNameDisplay">Thầy Cô</span>
            <span class="btn-edit-pen">✏️</span>
          </div>

          <select class="lesson-select" id="lessonDropdown" onchange="changeLesson(this.value)">
            <optgroup label="CHƯƠNG I: ESTER – LIPID">
              <option value="1">Bài 1: Ester – Lipid</option>
              <option value="2">Bài 2: Xà phòng và chất giặt rửa</option>
              <option value="3">Bài 3: Ôn tập chương 1 (Ester – Lipid)</option>
            </optgroup>
            <optgroup label="CHƯƠNG II: CARBOHYDRATE">
              <option value="4">Bài 4: Giới thiệu về carbohydrate. Glucose và fructose</option>
              <option value="5">Bài 5: Saccharose và maltose</option>
              <option value="6">Bài 6: Tinh bột và cellulose</option>
              <option value="7">Bài 7: Ôn tập chương 2 (Carbohydrate)</option>
            </optgroup>
            <optgroup label="CHƯƠNG III: HỢP CHẤT CHỨA NITROGEN">
              <option value="8">Bài 8: Amine</option>
              <option value="9">Bài 9: Amino acid và peptide</option>
              <option value="10">Bài 10: Protein và enzyme</option>
              <option value="11">Bài 11: Ôn tập chương 3 (Hợp chất chứa nitrogen)</option>
            </optgroup>
            <optgroup label="CHƯƠNG IV: POLYMER">
              <option value="12">Bài 12: Đại cương về polymer</option>
              <option value="13">Bài 13: Vật liệu polymer</option>
              <option value="14">Bài 14: Ôn tập chương 4 (Polymer)</option>
            </optgroup>
            <optgroup label="CHƯƠNG V: PIN ĐIỆN VÀ ĐIỆN PHÂN">
              <option value="15">Bài 15: Thế điện cực và nguồn điện hoá học</option>
              <option value="16">Bài 16: Điện phân</option>
              <option value="17">Bài 17: Ôn tập chương 5 (Pin điện và điện phân)</option>
            </optgroup>
            <optgroup label="CHƯƠNG VI: ĐẠI CƯƠNG VỀ KIM LOẠI">
              <option value="18">Bài 18: Cấu tạo và liên kết trong tinh thể kim loại</option>
              <option value="19">Bài 19: Tính chất vật lí và tính chất hoá học của kim loại</option>
              <option value="20">Bài 20: Tách kim loại và phương pháp tái chế kim loại</option>
              <option value="21">Bài 21: Hợp kim</option>
              <option value="22">Bài 22: Sự ăn mòn kim loại</option>
              <option value="23">Bài 23: Ôn tập chương 6 (Đại cương về kim loại)</option>
            </optgroup>
            <optgroup label="CHƯƠNG VII: NGUYÊN TỐ NHÓM IA VÀ NHÓM IIA">
              <option value="24">Bài 24: Nguyên tố nhóm IA</option>
              <option value="25">Bài 25: Nguyên tố nhóm IIA</option>
              <option value="26">Bài 26: Ôn tập chương 7 (Nhóm IA và IIA)</option>
            </optgroup>
            <optgroup label="CHƯƠNG VIII: DÃY KIM LOẠI CHUYỂN TIẾP THỨ NHẤT VÀ PHỨC CHẤT">
              <option value="27">Bài 27: Đại cương về kim loại chuyển tiếp dãy thứ nhất</option>
              <option value="28">Bài 28: Sơ lược về phức chất</option>
              <option value="29">Bài 29: Một số tính chất và ứng dụng của phức chất</option>
              <option value="30">Bài 30: Ôn tập chương 8 (Kim loại chuyển tiếp và phức chất)</option>
            </optgroup>
          </select>
          ...
          <button class="btn-quick-nav dash-btn" onclick="toggleDashboard()" title="Xem bảng điểm học bạ">📊 Học Bạ</button>
          <button class="btn-quick-nav pwa-install-btn" id="btnPwaInstall" style="display:none;" onclick="installPWA()" title="Cài đặt ứng dụng vào màn hình chính để dùng offline">📲 Cài App</button>
        </div>
        ...
        <div class="header-group">
          <div class="mode-tabs">
            ...
            <a href="./Dau_Truong_Hoa_Hoc_Lien_Nhom.html" class="btn-mode" style="background: linear-gradient(135deg, #f59e0b, #ef4444); color: #fff; text-decoration: none; display: inline-flex; align-items: center; border: none;" title="Chế độ thi đấu nhóm 4 tổ bằng QR Buzzer">⚔️ Đấu Trường</a>
          </div>
  ...
  <script>
    ...
    function goToNextLesson() {
      if (currentLessonId < 30) {
        changeLesson(currentLessonId + 1);
      } else {
        alert('Chúc mừng em đã hoàn thành tất cả 30 bài học của toàn bộ 8 Chương Hoá Học 12!');
      }
    }
    ...
    function showResults() {
      ...
      try {
        const key = 'HOA12_LESSON_SCORE_' + currentLessonId;
        const oldScore = parseFloat(SafeStorage.getItem(key) || '0');
        if (totalPoints > oldScore) {
          SafeStorage.setItem(key, totalPoints.toFixed(2));
        }
      } catch(e) {}
    ...
    function renderDashboard() {
      const chapters = [
        { name: 'CHƯƠNG I: ESTER – LIPID', start: 1, end: 3 },
        { name: 'CHƯƠNG II: CARBOHYDRATE', start: 4, end: 7 },
        { name: 'CHƯƠNG III: HỢP CHẤT CHỨA NITROGEN', start: 8, end: 11 },
        { name: 'CHƯƠNG IV: POLYMER', start: 12, end: 14 },
        { name: 'CHƯƠNG V: PIN ĐIỆN VÀ ĐIỆN PHÂN', start: 15, end: 17 },
        { name: 'CHƯƠNG VI: ĐẠI CƯƠNG VỀ KIM LOẠI', start: 18, end: 23 },
        { name: 'CHƯƠNG VII: NGUYÊN TỐ NHÓM IA VÀ NHÓM IIA', start: 24, end: 26 },
        { name: 'CHƯƠNG VIII: KIM LOẠI CHUYỂN TIẾP VÀ PHỨC CHẤT', start: 27, end: 30 }
      ];
      ...
      chapters.forEach(ch => {
        ...
        for (let id = ch.start; id <= ch.end; id++) {
          ...
          const scoreStr = SafeStorage.getItem('HOA12_LESSON_SCORE_' + id);
      ...
      document.getElementById('dashCompletedCount').textContent = completedCount + ' / 30';
    ...
    function resetProgress() {
      if (confirm('Em có chắc chắn muốn xóa tất cả lịch sử điểm số của 30 bài học để làm lại từ đầu không?')) {
        for (let i = 1; i <= 30; i++) {
          SafeStorage.removeItem('HOA12_LESSON_SCORE_' + i);
        }
        renderDashboard();
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      const urlParams = new URLSearchParams(window.location.search);
      if (urlParams.get('autoTest') === 'correct') {
        setTimeout(() => { selectMCQ(questions[0].ans); checkAnswer(0); }, 400);
      } else if (urlParams.get('autoTest') === 'dash') {
        setTimeout(() => { toggleDashboard(); }, 300);
      }
    });
  </script>
</body>
...
`;
  ...
  fs.writeFileSync('He_Thong_Trac_Nghiem_Hoa_Hoc_12.html', html, 'utf8');
  console.log('Successfully generated upgraded Master Hub (index.html & He_Thong_Trac_Nghiem_Hoa_Hoc_12.html)!');
}
...
<head>
  ...
  <meta name="mobile-web-app-capable" content="yes" />
  <meta name="theme-color" content="#070a14" />
  <title>Hoá Học 12 - ${lessonTitle}</title>
  <link rel="manifest" href="./manifest.json" />
  <link rel="apple-touch-icon" href="./icon-192.png" />
  <link rel="icon" type="image/png" sizes="192x192" href="./icon-192.png" />
  <link rel="icon" type="image/png" sizes="512x512" href="./icon-512.png" />
...
<body>
  ...
  <div class="container">
    <header>
      <div class="header-bar">
        ...
        <div class="header-group">
          <a href="./index.html" class="nav-btn-hub">← Cổng Tổng Hợp</a>
          <div class="teacher-badge" id="teacherBadge" onclick="openTeacherModal()" title="Nhấp để thay đổi tên giáo viên">
            <span>👨‍🏫</span>
            <span id="teacherNameDisplay">Thầy Cô</span>
            <span class="btn-edit-pen">✏️</span>
    ...
    <div class="result-card" id="resultCard">
      ...
      <div style="display: flex; justify-content: center; gap: 10px; flex-wrap: wrap;">
        <button class="btn-action" onclick="restartLesson()">Làm lại bài này ⟲</button>
        <a href="./He_Thong_Trac_Nghiem_Hoa_Hoc_12.html" class="btn-action btn-next-step" style="text-decoration:none;">Về Cổng Tổng Hợp ➔</a>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function showResults() {
      ...
      try {
        const key = 'HOA12_LESSON_SCORE_' + LESSON_ID;
        const oldScore = parseFloat(SafeStorage.getItem(key) || '0');
        if (totalPoints > oldScore) {
          SafeStorage.setItem(key, totalPoints.toFixed(2));
        }
